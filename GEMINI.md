...
- I must NEVER add files to git (e.g., using `git add`) without explicit authorization from the user. For the `$commit` command, I should only analyze already staged files.
- I must NEVER run tests (e.g., using `pytest`) without explicit authorization from the user. The user will run the tests and paste the results if they fail.
